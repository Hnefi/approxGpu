...
ImagePyramid* createOutputImages(I2D* imageIn,cudaTextureObject_t* texObj)
{
    int rows, cols;
    rows = imageIn->height;
    cols = imageIn->width;
    ...
    int weightedKernel[5] = {6,1,2,1,6};
    ...
    resizeKernel_st1<<<nblocks,threadsPerBlock,bytesForSmem>>>(d_origInput,resizeInt,d_weightedKernel,rows,cols,resizedRows,resizedCols,objToKernel);
    ...
    calcSobel_dX_k1<<<nblocks,threadsPerBlock,bytesForSmem>>>(d_origInput,dxInt,sobel_kern_1,sobel_kern_2,cols,rows,objToKernel);
    calcSobel_dX_k2<<<nblocks,threadsPerBlock,bytesForSmem>>>(dxInt,dxOutput,sobel_kern_1,sobel_kern_2,cols,rows,objToKernel);

    calcSobel_dY_k1<<<nblocks,threadsPerBlock,bytesForSmem>>>(d_origInput,dyInt,sobel_kern_1,sobel_kern_2,cols,rows,objToKernel);
